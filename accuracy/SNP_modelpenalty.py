...
def run_bwa(files,database,tempbamoutput):
    ...
    if include_secondary:
        if penalty[0] > 2:
            cmds += '/usr/bin/time -v bwa mem -a -t %s -B %s -O %s -E %s -A %s %s %s > %s.all.sam\n' % (
                min(40, args.t), penalty[0] - 2, penalty[1] -2, penalty[2],  penalty[3] + 2, database, files,  tempbamoutput)
        else:
            cmds += '/usr/bin/time -v bwa mem -a -t %s -B %s -O %s -E %s -A %s %s %s > %s.all.sam\n' % (
                min(40, args.t), penalty[0] - 0.5, penalty[1] - 0.5, penalty[2], penalty[3] + 0.5, database, files,
...
def run_mapper(files,database,tempbamoutput):
    ...
    cmds += '/usr/bin/time -v java -Xms55g -Xmx55g -jar %s --no-infer-ancestors --additional-extend-insertion-penalty 0  --max-penalty-span 0 --max-penalty %.2f --snp-penalty %s --new-indel-penalty %s --extend-indel-penalty %s --ambiguity-penalty %s --num-threads %s --reference %s --queries %s  --out-sam %s.sam\n' % (
        ...
        tempbamoutput)
    for k in range(12,26,2):
        cmds += '/usr/bin/time -v java -Xms55g -Xmx55g -jar %s --no-infer-ancestors --additional-extend-insertion-penalty 0  --max-penalty-span 0 --block-length %s --max-penalty %.2f --snp-penalty %s --new-indel-penalty %s --extend-indel-penalty %s --ambiguity-penalty %s --num-threads %s --reference %s --queries %s  --out-sam %s.kmer%s.sam\n' % (
            latest_mapper_kmer, k, max_penalty, penalty2[0], penalty2[1], penalty2[2], penalty2[3], min(40, args.t), database,
            files,
            tempbamoutput,k)
    # cmds += '/usr/bin/time -v java -Xms55g -Xmx55g -jar %s --no-infer-ancestors --additional-extend-insertion-penalty 0  --max-penalty-span 0 --block-length 20 --max-penalty %.2f --snp-penalty %s --new-indel-penalty %s --extend-indel-penalty %s --ambiguity-penalty %s --num-threads %s --reference %s --queries %s  --out-sam %s.kmer20.sam\n' % (
    #     latest_mapper_kmer, max_penalty, penalty2[0], penalty2[1], penalty2[2], penalty2[3], min(40, args.t), database,
    #     files,
    #     tempbamoutput)
    # cmds += '/usr/bin/time -v java -Xms55g -Xmx55g -jar %s --no-infer-ancestors --additional-extend-insertion-penalty 0  --max-penalty-span 0 --block-length 25 --max-penalty %.2f --snp-penalty %s --new-indel-penalty %s --extend-indel-penalty %s --ambiguity-penalty %s --num-threads %s --reference %s --queries %s  --out-sam %s.kmer25.sam\n' % (
    #     latest_mapper_kmer, max_penalty, penalty2[0], penalty2[1], penalty2[2], penalty2[3], min(40, args.t), database,
    #     files,
    #     tempbamoutput)
    if tool == 'bwa':
